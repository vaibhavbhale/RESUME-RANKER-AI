...
from apps.resumes.services.extraction import extract_text_from_upload
...
def result_detail(request, result_id: int):
    result = get_object_or_404(
        ...
        id=result_id, batch__created_by=request.user
    )
...
def resume_preview(request):
    ...
    if not files:
        return JsonResponse({"items": [], "error": "No files received"}, status=400)

    items = []
    for f in files:
        try:
            text = extract_text_from_upload(f)
            parsed = parse_resume_heuristic(text)
            years = parsed.get("total_years_experience", None)
            level = "Fresher" if (years is None or float(years) <= 0.5) else "Experienced"
            items.append({"filename": f.name, "years": years, "level": level, "skills": (parsed.get("skills") or [])[:10]})
        except Exception as e:
            items.append({"filename": getattr(f, "name", "unknown"), "error": str(e)})
